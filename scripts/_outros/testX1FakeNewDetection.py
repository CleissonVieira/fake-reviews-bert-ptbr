# https://colab.research.google.com/drive/1JT0G0L2xYx_CGQ0d8h1SFuA5yOlJLeYf#scrollTo=BEG81NvL1Rt9
...
patch = f'../resultadoScripts/testB2FakeNewDetection-{datetime.now().strftime("%Y-%m-%d_%H-%M")}/'
...
data = pd.concat([true_data, fake_data]).reset_index().drop(columns=['index'])
...
def evaluate(dataloader):
    ...
    for step,batch in enumerate(dataloader):    # Iterate over batches
            ...
            with torch.no_grad():                         # Deactivate autograd
                ...
                all_preds.extend(np.argmax(preds_cpu, axis=1))
...
for epoch in range(_epochs):
    ...
    if valid_loss < best_valid_loss:              # save the best model
        ...
        torch.save(model.state_dict(), f'{patch}model.pt')
...
model.load_state_dict(torch.load(f'{patch}model.pth', weights_only=True))
model.to(device)
